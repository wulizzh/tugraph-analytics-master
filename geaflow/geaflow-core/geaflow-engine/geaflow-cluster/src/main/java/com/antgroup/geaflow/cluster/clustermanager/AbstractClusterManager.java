...
public abstract class AbstractClusterManager implements IClusterManager {
    ...
    //init 方法用于初始化集群管理器。
    //接受一个 ClusterContext 对象作为参数，从中获取集群配置、上下文等信息进行初始化

    public ClusterContext getClusterContext() {
        return clusterContext;
    }

    protected abstract IFailoverStrategy buildFoStrategy();//抽象方法 buildFoStrategy()，由子类实现，用于构建故障转移策略
    ...
    //allocateWorkers 方法用于分配工作节点。
    //根据工作节点数量，计算所需的容器数量并启动相应数量的容器
    ...
    //startContainers 方法用于启动容器。
    //循环启动指定数量的容器，并将容器 ID 添加到容器 ID 集合中
    ...
    //startDriver 方法用于启动驱动节点。
    //创建并启动一个驱动节点，并返回其 RPC 地址。

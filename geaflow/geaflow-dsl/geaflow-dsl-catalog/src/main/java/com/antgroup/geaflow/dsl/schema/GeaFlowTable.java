...
public class GeaFlowTable extends AbstractTable implements Serializable {
    ...
    private final List<String> primaryFields;// 主键字段列表。

    private final List<String> partitionFields;//分区字段列表。

    private final Map<String, String> config;//保存了表的配置信息
    ...
    //以上五个分别返回实例名、表名、字段列表、主键字段列表、分区字段列表、配置信息、是否存在标志以及是否是临时表标志。
